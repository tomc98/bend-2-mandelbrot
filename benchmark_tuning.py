...
cases=[('deep320',Fraction(0),Fraction(1),Fraction('1e-68'),16384),('seahorse',Fraction('-.743643887037151'),Fraction('.131825904205330'),Fraction('.008'),1222),('generic_deep',Fraction('-.743643887037151'),Fraction('.131825904205330'),Fraction('1e-7'),2785),('deep300',Fraction(0),Fraction(1),Fraction('1e-300'),16384)]
...
try:
    with ThreadPoolExecutor(max_workers=3) as pool:
        for name,cx,cy,span,it in cases:
            ...
            hashes=set()
            for root,count in [(r,c) for r in (256,512,512,256) for c in (1,2,3)]:
                ...
                row=dict(name=name,root=root,workers=count,total_ms=ms,batch_p50_ms=statistics.median(times),batch_p95_ms=sorted(times)[int(len(times)*.95)],sha256=digest)
                rows.append(row);print(json.dumps(row),flush=True)
            assert len(hashes)==1
finally:
    for p in workers:p.stdin.close()
    for p in workers:p.wait(timeout=20)
(ROOT/'evidence/acceleration/tuning.json').write_text(json.dumps(rows,indent=2)+'\n')
