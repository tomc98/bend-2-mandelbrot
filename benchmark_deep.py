...
args=argparse.ArgumentParser();args.add_argument('baseline');args.add_argument('--output',default='evidence/deep-performance/pixel-parallel.json');options=args.parse_args()
w,h=2200,1560
cases=[('generic',Fraction('-.743643887037151'),Fraction('.131825904205330'),Fraction('1e-7'),2785),('deep300',Fraction(0),Fraction(1),Fraction('1e-300'),16384)]
paths=[Path(options.baseline),ROOT/'Mandelbrot.app/Contents/MacOS/worker'];rows=[]
logs=[open(ROOT/f'evidence/deep-performance/worker-{i}.log','w') for i in range(2)]
